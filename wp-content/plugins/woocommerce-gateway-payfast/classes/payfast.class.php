...
class WC_Gateway_PayFast extends WC_Payment_Gateway {
	...
	public function __construct() {
        global $woocommerce;
        $this->id			= 'payfast';
        $this->method_title = __( 'PayFast', 'woothemes' );
		...
		add_action( 'valid-payfast-standard-itn-request', array( $this, 'successful_request' ) );
    ...
    function init_form_fields () {
    	...
    	$this->form_fields = array(
    						'enabled' => array(
											...
											'label' => __( 'Enable PayFast', 'woothemes' ),
											'type' => 'checkbox',
							...
							'send_debug_email' => array(
											...
											'type' => 'checkbox',
											'label' => __( 'Send debug e-mails for transactions through the PayFast gateway (sends on successful transaction as well).', 'woothemes' ),
											'default' => 'yes'
										),
							'debug_email' => array(
    ...
    function add_testmode_admin_settings_notice () {
    	$this->form_fields['merchant_id']['description'] .= ' <strong>' . __( 'PayFast Sandbox Merchant ID currently in use.', 'woothemes' ) . ' ( 10000100 )</strong>';
    	$this->form_fields['merchant_key']['description'] .= ' <strong>' . __( 'PayFast Sandbox Merchant Key currently in use.', 'woothemes' ) . ' ( 46f0cd694581a )</strong>';
    ...
    public function generate_payfast_form( $order_id ) {
		...
		return '<form action="' . $this->url . '" method="post" id="payfast_payment_form">
				...
				<input type="submit" class="button-alt" id="submit_payfast_payment_form" value="' . __( 'Pay via PayFast', 'woothemes' ) . '" /> <a class="button cancel" href="' . $order->get_cancel_order_url() . '">' . __( 'Cancel order &amp; restore cart', 'woothemes' ) . '</a>
